...
open class CornerGradientView2: UIView
{
  ...
  @IBInspectable open var fadeIntensity: Float = 0.0
    ...
    @IBInspectable open var cornerRadius: Float = 0.0
      {
      didSet
      {
        assert(cornerRadius >= 0, "Corner Radius must greater than 0")
        if gradientLayer != nil
        {
  ...
  override open func draw(_ rect: CGRect)
  {
    ...
    self.updateLocation()
    self.updateCornerRadius()
    gradientLayer!.shadowColor = UIColor(red: 90/255.0, green: 15/255.0, blue: 107/255.0, alpha: 1.0).cgColor
    gradientLayer!.shadowOpacity = 0.2
    gradientLayer!.shadowOffset = CGSize(width: 20, height: 5)
  ...
  {
    gradientLayer!.colors = [UIColor(red: 197/255.0, green: 90/255.0, blue: 226/255.0, alpha: 1.0).cgColor, UIColor(red: 226/255.0, green: 100/255.0, blue: 224/255.0, alpha: 1.0).cgColor]
  }
  /**
   Step 2
   */
